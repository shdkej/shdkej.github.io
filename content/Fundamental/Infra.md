...
#### 멀티 클라우드로 내 생각을 확장해야 할까 아니면 단일 클라우드에 락인이 되도 될까
- 멀티 클라우드의 복잡성까지는 내가 쓸 일이 없을 것 같다
- 그렇지만 락인은 항상 경계해왔다
- 하지만 AWS 등 단일 클라우드도 이미 생태계가 충분히 성숙해졌다
- AWS에 종속되면 GCP를 못쓰나? 그건 아니다
- 근데 AWS를 쓰는 회사가 GCP를 쓰려고 한다? 그런 일은 잘 없을 것 같다
...
- s3 gateway endpoint 고려 (내부 통신용)
- eks egress 통신 확인 (nat 통신은 비싸다)
- dynamodb는 월 사용량이 일정하면 provisioned
...
#### 백엔드는 프라이빗에 두고 프론트만 퍼블릭에 두기
- 프론트는 브라우저 요청이니까 백엔드 주소 접근을 해야할때 못함. 그래서 프록시를 프론트에 두면 서버 to 서버는 가능
- 근데 그래봤자 프록시 주소로 뚫리면 백엔드에 접근 가능한거 아닌가
	- 맞다. 하지만 프론트에서 접근제어를 한번 해준다는게 중요한 장벽임. 이를 이용해 api 주소를 바꿔도 되고 프론트에서 차단해도 됨.
- 그럼 프록시를 하면 단계를 하나 더 거치니까 부하가 더 들 수 있지 않나. 캐싱이라던가 문제가 생길 수 있을 거 같은데
	- 헤더나 캐시나 그냥 쓰루패스 하면 되긴 함
- 백엔드를 프라이빗 파드에 두고 ingress를 공통으로 둬서 프라이빗에 접근하는 방식으로 쓰면 된다
...
- RDS 디비 접속
	- 서브넷이 private용으로 돼있는데 security group 만 풀어주면 접속이 되나? 아니면 서브넷도 풀어줘야 하나?
...
- 버전 확인해서 업데이트 필요
	...
	- lb controller
- 설치 순서
	1. argo, ingress, prometheus, kop 리소스, 모니터링 
	2. kop 리소스 -> hasura, admin-api, api, cms, web,
