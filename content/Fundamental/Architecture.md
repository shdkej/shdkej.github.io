...
- 객체가 2개 이상 생기면 중간에 버퍼가 들어가면 안정적이다. 근데 DB와 웹은 하나로 봐야하려나

## 설계
- 서비스 유형 - 어떤 도메인에서 어떤 비즈니스를 하는지, 그것에 대한 특성
- 개인정보 처리
- 데이터 양 측정
- IaC
...
#### 회사 인프라 최초 세팅 시
내가 스타트업을 시작한다면?
- 시스템 인프라를 추가한다 (모니터링, 로깅, CD툴)
- 지표, 통계를 확인할 수 있도록 한다 - GA, Log Analysis Report
- 배포를 자동화한다
- 환경 분리 - 스테이지/운영
- 피처 플래그 개발 환경 구축
- 유저 관리를 인프라로 할지 서비스별로 할지
...
- [x] HTTPS도 공개키 방식처럼 암호화를 개인키로 하면 비밀키로 복호화를 해야하는 방식인가? ✅ 2026-07-04
...
- [x] 비밀키를 서버가 갖고, 공개키는 아무나 갖는다. 근데 CI에서 비밀키를 가지는 것은 어떻게 생각해야하지? 브라우저에서는 서버가 비밀키를 갖는다. ✅ 2026-07-04
	- ci 서버가 비밀키를 갖는 경우는?
...
- 서버가 자신임을 증명해야 하는 이유는?
  ...
  - 중간에 데이터가 변경되었거나 이상한 데로 요청하는 걸 막고 (cors)
  - 내가 들어온 사이트가 내가 들어가려했던 그 서버가 맞는지 인정받기 위해
- [x] HTTPS는 국가에서 막기 힘든 이유는? ✅ 2026-07-04
