...
- [x] 어떤 것을 보완하기 위해 모놀리스라는 아키텍처가 달성되었을까 ✅ 2026-06-16
- 3 layer 아키텍처를 만들다보니 모놀리스가 된 것인가
- 모놀리스는 어떤 것의 합이었다기 보다는 흩어진 데이터, 실행 등을 하나로 묶어 복잡도를 피하려 나온 기본값이다
- 이후 프로그램의 규모와 팀의 규모가 커짐에 따라 그 응집이 병목이 되었다
