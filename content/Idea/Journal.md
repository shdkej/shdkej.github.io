...
#### 노트 2.0
- 개발자에서 생산자로 전직하면서 노트의 구조도 바뀔 때가 왔다
- 만다라트 구조와 카테고리는 유효하다
- 기본기에 들어가야할 내용 등이 달라진다
- 합칠건 합치고 새로 만들건 새로 만든다

내 노트에서의 장단점 및 개선점 받기
- 질문은 많이 쌓였는데 무엇이 현재 결론인지 한눈에 드러나지 않아 재사용 효율이 떨어짐
- 노트 압축 필요. 또는 발행된 글과 아닌 글 분리 필요. 인사이트를 활용 하는 내가 되는 노트가 되도록
...
2. 간결한 문장
3. 결론 먼저 (두괄식)
4. 경험(정성적)과 감정으로 기억에 남게 할수있다
5. 유머가 있어야 한다
6. 비유로 설명하는 것은 좋은 방법
7. 글에 호기심을 담으면 흡입력이 강해진다
8. 일관된 폼

#### 기타 작성 요령

- 첫줄에 한줄요약으로 그림을 그릴 수 있도록, 연상을 할 수 있도록 작성. 그 뒤에 질문과 설명 적기
...
#### 노트 정리법
- 내 노트는 한 주제에 다양한 내용이 들어간다
- 일단 많은 내용을 정리하는 방법은 분류 편집 지우기이다
- 오늘 목차를 나눠놓는다
- 목차는 10개를 넘기지 않는다
- 불릿이 8개를 넘으면 소재가 되고 글이 되게 한다
- 글을 보기 좋게 편집한다
...
회사에 처음으로 코드를 수정하는거라 최대한 안전하게 하고 싶어서 기존 코드를 그대로 따랐는데, 너무 그대로 따랐다. 리팩토링할 여지가 충분히 있었는데, 미뤘다.
처음 온 개발자가 만든 코드든, 베테랑이 만든 코드든 운영에는 똑같이 돌아간다.
그러니 코드에 처음이니까 괜찮겠지는 없다.
그리고 코드 하나를 수정하는데에도 이유가 있어야 하고, 그 분기를 하게 되면 그것에 대한 내용도 파악하고 있어야 한다. 한 부분을 변경하더라도 다른 부분이 영향을 받는지 신경을 계속 써야한다. 내가 맡은 부분만 신경쓰려고 하고, 주변 사이드 이펙트는 없겠지 하고 넘어가려고 했는데, 더 주의를 기울여야겠다.
...
- [x] 출처를 알 수 없는 인용구는 어떻게 표시할까 ✅ 2026-06-29
...
#### 퇴고할 수 있는 노트 환경을 만들어야 한다
정해진 분량을 정해놓던가 - 기본, 심화
기간을 정해놓던가 - 리뷰
Meta에 있는 것들을 정리하면서 기술적인 부분을 좀 정리하자
마지막 수정일자를 확인해서 오버데이트된건 좀 확인하도록 하자 대략 1년 이내로 최신 관리하자


#### 노트 구분
기본기 운영체제, 깊이 보안, 다른분야 심리학, 교육, 윤리-법 추천
교육은 지식 전달 능력

추가로 필요할 것
- 분산 시스템 이론 비동기 객체지향 클린코드
- api design
- authentication
- data pipeline
...
노트를 두터워지게 만들고 싶다
- meta는 장차 다른 노트들에 흡수되어야 한다
- 특정 주제에 집중하고 싶으면 문서를 따로 딴다
- 벤치마크는 어느 카테고리로 -> monitoring
- 노트 연도별 정리 되는거
	- reading
	- movie
	- think
	...
	- travel
	- journal
	- information 등 idea 에 있는 거
	- archive
	- collection
	- curiosity
	- inbox

#### 다른 주제에 대한 노트를 시각적으로 어떻게 구분하면 좋을까
- 샾이 있고
- 괄호가 있고
- 들여쓰기가 있고
- 문단 나누기가 있고

샾으로 나눈 하나의 주제가 너무 길어지면 다른 것도 한눈에 안들어오게 된다

#### 관리법
- 10000자 넘는 문서
	- [[Reading]]
- 데일리 노트에는 카테고리화 안되는 것만 남기고 최대한 분류 작업을 한다.
	- 내용은 중복되도 된다.
	- 내용은 삭제되어도 된다.
- 육각형을 키운다
- 노트를 가치있게 만든다
...
- 긴 노트는 얼추 정리 됐고 이제 한 눈에 주제가 다 들어오는지 확인해봐야겠다
	- 크게 어떤 주제들로 이루어진 문서인지 알아야 한다.
- 지금 옵시디언 테마 강조표시랑 코드가 일반 글씨랑 구분이 잘 안됨
	- 전체적인 색감과 자간, 탭의 느낌은 마음에 듦
- 블로그에서 죽은 링크 찾아주는 서비스 없나
- vs를 쓸 때는 그냥 줄바꿈해도 된다. 렌더링 될때는 한줄로 보임
- 구글 포토 이미지르 다 여기로 가져오고 싶다. 그러려면 일단 기존의 이미지를 다시 띄우고 싶다
	- 폴더경로로 바뀌면서 상대경로를 바꿔줘야했음
...
- [x] 글, 노트, 업무기록을 구조화, 시각화 하고 싶다 ✅ 2026-06-29
- [x] 글쓰기 스킬 (다의어 함축어를 명료하게 검토해주기?, 기본 글쓰기 규칙 적용되게) ✅ 2026-08-27
- [x] 다이어리에 쓴걸 캘린더에 넣어서 1년마다 리마인더 할 수 있을까 ✅ 2026-06-29
...
- [x] 추적과 그래프. 습관 - 습관 추적에 그래프 쓰는 노션 페이지 좋아보였음 ✅ 2026-07-04
- [x] 캘린더로 계획을 세워서 그걸 지키는게 목표가 아니라 계획한걸 실제로 어떻게 했는지 확인하고 변경하면서 어떻게 진행되어왔는지 확인하는 식으로 쓰면 계획에 쫓기지 않을 수 있겠다 🔺 ✅ 2026-05-31
- [x] PARA 로 내 폴더 구조를 매핑 할 수 있을까 🔽 ✅ 2026-07-04
- [ ] 북마크에 있는 것들을 노트에 녹이기🔽 
- [ ] 기본기 잘 요약되어있는 블로그 연결 🔽 
- [ ] 운송수단과 인덱스 구조 매핑. 비행기 - 기차 - 버스 - 걷기의 레이어로 접근되는 스케일이 다름
