...
뭐가 깔렸는지 쉽게 알고 다른 환경으로 이동했을 때 제약없이 옮길 수 있는 프로그램을 원해서 하나의 프로그램에 종속되는 프로그램이 있으면 안쓰고 있었는데 돌아봤을 때 적게 깐다고 다 기억할 수 있는 것도 아니고 많이 깐다고 필요한 프로그램을 아예 안쓴것도 아니다 적은 종속성은 좋지만 마음대로 까는 것도 필요하겠다 싶다 초기화 후 원래 상태로 복구하는 것을 쉽게 하기 위해 설치를 많이 안했다
...
#### infra
코어로직이라도 변경하기 쉽게 할 수 있을까? 변경하기 힘든 이유는 영향갈만한게 많을거 같아서? 아니면 도메인적으로 이미 묶여있는게 많아서? 설계할때부터 그런게 없도록 설계해야하나?
