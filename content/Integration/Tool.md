...
#### tailscale
- sudo tailscale up --ssh 로 실행해줘야 ssh 접속 가능
- 6개월이면 키 만료던데 업데이트는 어떻게 하지?
	- 키 만료를 끌 수도 있긴 있다
	- 만료 전에 이메일 와서 리프레시할 수 있다고 알려준다
	- 리프레시 되면 서버에 설정 다시 해줘야하나?
	- `sudo tailscale up --force-reauth`
#### openclaw 활용법
- kubernetes 서버 띄우는 곳에서 쓸거라 이거 모니터링용
- cron 으로 뉴스 수집?
- 근데 브라우저 접속은 안됨
- 터미널 활용법
- 헤드리스 playwright 써보기
- github의 내 노트 접근
- openclaw 대시보드 접속
	- 서버에서 openclaw dashboard
	- 내 컴퓨터에서 `ssh -N -L 18789:127.0.0.1:18789 ubuntu@100.96.204.36`
	- 켜놓고 내 컴퓨터에서 브라우저 접속
- 모델 어떻게 바꿨더라. 다시 세팅했네
...
#### Terraform 단점
- 버전 업그레이드 시 변경사항이 plan에 같이 떠서 plan 자체를 보기 힘듬
...
- [x] grpc호출하려면 grpc호출 로직을 짜야하나? 간단하게 호출할 수 있는 방법은? ✅ 2026-03-18
