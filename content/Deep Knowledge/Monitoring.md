---
title: 모니터링
summary: 관측가능성과 피드백루프
date: 2025-07-15 15:35:32 +0100
updated: 2026-05-30 07:42:06
tags: deep_knowledge
---
...
#### 글로벌 로깅 구조 잡아야하나 말아야하나
트레이싱이 이게필요
로깅은 비즈니스로직 영역에서 추가하고
요청과 응답을 정확하게 확인할 수 있도록만 api 구성
...
#### 변화율 지표를 나타낼 방법
- 비용 측정을 할 때 EC2는 사용량이 많은 서비스라 300만원이 나오고 SNS는 비용이 5만원 정도 나온다고 했을 때 두 가지를 바 차트에서 보면 너무 차이 나 보인다
- 이걸 같은 비율의 차트로 보는 방법은? 예를 들어 EC2는 300만원이 기준값 1이고 SNS는 5만원이 1로 봤을 때 여기서 비율만큼 늘어나면 이상감지 신호를 보내게 한다면 어떤 수식으로 만들어야 하는가
- 직관적인 방법 : 현재값 / 기준값, 330만원 / 300 = 1.1
- 더 정교한 방법 : (현재값 - 과거 평균 비용) /  과거 비용의 표준편차

#### 로그를 코드에 일일이 넣어야 하나? 다른 방법이 있나?
- 이건 애플리케이션에서 해줘야 할 듯
- 트레이싱과 로깅
- 어느정도는 앱에 해둬야 하고 분산 환경에서 추적을 위해 인프라에서도 트레이싱 해줘야 함
...
## Alert
- 알림은 정말 중요한 상황에서만 와야한다 (피로도 관리)
- 무시해도 되는 알림이 쌓이면 중요한 알림을 놓치게 된다
- 메신져에서는 중요한 대화방과 아닌 방을 나눠야 알림에 의해 방해를 덜 받겠다.
- 슬랙 알림 시 제목을 딱 표시나게 한줄로 해놔야 미리보기에서 뭔지 눈에 딱 들어오겠다
...
- AWS는?
- 앱 하나 추가해서 확인해보기
- grafana, loki, prometheus도 잘 파야겠다
	- 로키를 잘 써서 로그를 쉽게 볼 수 있도록
	- grafana에 기본 템플릿 만들어놓기
	- prometheus에 데이터가 계속 쌓이면 어떻게 관리되야하지? 지표는 중요한데
	- eks에서 내가 보고싶은 노드를 정확하게 셀렉하는 좋은 방법은?
	- loki랑 fluentbit 연결하는 방법은?
	- loki에서 조회했던 쿼리들 저장은 대시보드로만 할 수 있나

## prometheus

- prometheus 자체는 중앙에 1대 띄워놓는 거다
- /metrics를 얘가 조회해서 데이터 가져가는 방식
- nextjs나 fastapi나 라이브러리 설치해서 코드에 이 경로를 노출하도록 해줘야함
- node-exporter
...
- 모니터링 적용하기
	- 기존 github을 그대로 쓸수 없고 설정파일을 바꿔줘야한다
	- 설정은 monitoring에 셋업은 space에 있었으면 좋겠다
	- 리소스 제한은 살려줬으면 좋겠다
	- space 업데이트 어떻게 하지?
		- 배포하면 terraform 실행 되게 함
	- 모니터링 시스템 다시 세팅해야함
		- blackbox 대신 node exporter -> 이것도 자동 내장
		- cadviser 확인 -> 이건 k8s면 kubelet에 내장되어있다고 함. 일단 제외
	- 지표들이 mock인게 많았다 하나씩 다시 세팅 필요
		- 일단 비즈니스쪽 하루로 안되서 30일로 모니터링 세팅함
	- 람다는 따로 모니터링 페이지를 둬야하려나
	- supabase, dynamodb
	- 모든 레이어 중 하나씩만 보는 메인 대시보드 하나 만들기
	- 대시보드에 목표치가 있어야겠다 -> 이거는 알럿으로 걸어두면 되겠다
	- 비즈니스 지표에 얼마를 벌었는지나 AARRR이 없는거같다
	- 프로덕트는 북극성지표가 프로덕트마다 따로 있을 수 있다