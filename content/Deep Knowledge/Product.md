...
#### 기술 민주화, 디지털 문해력. 내가 해결하고 싶은 키워드
불평등의 거부를 하려는 인간의 습성
가치 분류 : 불평등 해소, 지속가능성 

역사 : 공유지를 사유지로 만드는 것에 대한 저항

#### 프로덕트에 담길 내 철학

변화량이 중요하다 - 나의 변화량 독자의 변화량
아날로그가 뜬다 아날로그한 물건의 부수적인 물건을 만들면 어떨까 예를 들면 책갈피

미켈란젤로 - 작품을 잃어도 가치관을 지킴. 작품에 대한 미련이 없음
라파엘로 - 아테네학당, 철학역사를 훑고 다른 사람들의 기술을 자기에게 흡수함
...
#### 대학생 수준이나 소수의 작은 팀 회사 수준이 아니라 피그마가 되려면, 노션이 되려면 어떤 것이 필요할까
- 데모가 끝이 아닌 지속적 관리
- 기존 솔루션보다 더 낫게 계속 해결
- 누군가가 시간, 돈을 계속 잃고 있는 문제를 찾는다
- 고객 접촉이 사후 관리까지 이어진다
- 제품이 실패했을 때 대처까지 고려한다
- 다음 사용자를 데려올 수 있는 기능을 넣는다
- 이 제품이 누군가의 일을 계속 대체할만큼 믿을 수 있는가


#### 고객 중심
> 창업자들은 뛰어났고 제품도 진짜 새로웠습니다. 그런데 피치(투자자 앞에서 하는 제품 설명)를 시작할 때마다 아키텍처부터, 자기들이 자랑스러워하는 영리한 부분부터 꺼냈어요. **이야기에서 고객의 고통이 삭제**돼 있으니 그냥 소음으로 들렸습니다. 그래서 사용자가 진저리를 내던 바로 그 지점을 도입부에 다시 넣었어요. 같은 제품이고 같은 주였는데, 그다음 대화들이 파일럿으로 전환됐습니다. 그리고 그걸 반복 가능한 고투마켓 시스템으로 만들었고요
- 고객 중심이 시작이자 끝이다
...
브랜드가 먼저인가 프로덕트가 먼저인가
- 프로덕트
- 근데 미니멀모으기는 브랜드를 같이 하는 것
- 좋지 뭐
...
#### 게리 탄 조쉬 뉴스레터
**==YC 파트너들이 창업자의 야망을 한 단계 끌어올릴 때 쓰는 화법이 눈에 보이는 겁니다.==**
**==에이전트가 주어진 작업에 대해 어떤 100만 토큰을 머릿속에 담고 있어야 하는지를 알게 해 주는 겁니다.==**
요청 하나에 100만 토큰이든 80만 토큰이든 넣어 달라고 하고, 그게 soul.md(에이전트의 정체성과 배경 지식을 담아 두는 파일)에 들어가 있게 만드는 거죠.
...
Think → Plan → Build → Review → Test → Ship → Reflect Each skill feeds into the next. /office-hours writes a design doc that /plan-ceo-review reads. /plan-eng- review writes a test plan that /qa picks up. / review catches bugs that /ship verifies are fixed. Nothing falls
through the cracks because every step knows what came before it.

#### 허진호님의 two cents
https://twocents.xyz/p/two-cents-97-consumer-ai-2-yahoo?utm_source=post-email-title&publication_id=19020&post_id=211961966&utm_campaign=email-post-title&isFreemail=true&r=3z2by&triedRedirect=true&utm_medium=email
AI를 매개로 하여 소비자가 “물건을 찾고 구매하는 과정”, 좀더 본질적으로는 “소비자가 일상 생활/소비에 필요한 것들을 공급받는 과정” (일상 용품, 취미 생활, 패션 등을 모두 포함하여) 자체가 근본적으로 바뀌고, 그 변화가 장기적으로 지속하는한 “새로운 소비자 behavior”가 만들어지는 과정이 필요하다
...
웹 시대의 소비자 접점을 만들며 겪었던 시행착오들을 참고하여, AI 시대의 소비자 접점 플랫폼에서는 그 시행착오를 반복하지 않으면서 소비자들을 모을 수 있는 구조를 만드는 새로운 기회의 창이 열린 것이 아닐까 한다
...
- 프리랜서이기 때문에 이것저것 잡다하게 일을 해야한다 하지만 원칙을 세워 방향을 잡고 하지 않을 것도 정한다
	- 할 것
		- 지속 가능한가
		- 방향성이 맞는가
		- 확장 가능한가
...
- 내가 가야할 시장
	- https://twocents.xyz/p/two-cents-97-consumer-ai-2-yahoo?utm_source=post-email-title&publication_id=19020&post_id=211961966&utm_campaign=email-post-title&isFreemail=true&r=3z2by&triedRedirect=true&utm_medium=email
- 하지 않을 것
	- 매시브 비즈니스와 스몰 비즈니스는 차이가 있다 위챗을 만드려고 하지 않는다
	- 맨투맨이 미래인데 이걸 고부가가치로 만드려면? 일일이 코칭 및 상담은 비용도 그렇고 효율적이지 않다
