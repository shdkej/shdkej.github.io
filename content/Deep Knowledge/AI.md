...
세팅
	- ln -s TECH_SPEC.md
	- ln -s LLM.md CLAUDE.md
	- 워크플로우 -> 에이전트로 세팅
	- 서브에이전트는 스킬을 쓰려면 명시해줘야함
	- 키 보안은 어떻게 관리하지 env 읽는건 괜찮은건가?
	- slack mcp는 node를 써서 kop-web에서 안돎..
		- ASDF_NODEJS_VERSION=18.20.8 node 를 쓸 수 있다
	- [ ] 슬랙 배포 후 완료 확인 에이전트로 처리 필요
	- mcp 절약하는 법 (매번 호출 안하는 법)
		- mcp를 skills 를 쓸 때만 호출하게 할 수 있나
	- claude 상태창에 사용량 확인하는 법
