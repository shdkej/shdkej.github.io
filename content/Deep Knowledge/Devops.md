...
#### 데브옵스의 필요성
작은 기업에서는 기획,개발,qa,데브옵스를 나누지 않을 것이고 규모가 커져야 qa엔지니어의 필요성을 느끼는 듯싶다
...
관리 페이지에서 infra 요청을 받고 자동으로 할당 받게 한다.
...
- 데브옵스로서 자료를 모으고 공유하는 방법  
	- 흩어진 자료를 모아서 보기 좋게 만드는 작업  
	- 모르는 부분을 쉽게 검색할수있는 환경
...
CI 서버와 CD 서버를 나눌 필요가 있을까 -> CD는 배포된 내역을 관리하는 대시보드의 필요성이 있다
...
버저닝 시 pr마다 버전을 넣으면 금방 100을 넘을 것 같은데? 컨테이너 이미지 만들 때 버저닝 같이 하게 된다
...
- CD 파이프라인
	- CI 에서 배포된 내용을 관리 및 모니터링

- 레포지토리 관리 AI 봇
	- 버전 관리 -> 이건 작업 내용과는 상관 없어서 따로 이미지 만들어서 처리해야 할 듯
	- 성능 관리 (실행 시간, CI 시간 등)
	- 모니터링 지표 관리
	- 문서화
	- 이미지 관리
...
- 연계된 서비스의 배포는 어떻게 하고 QA를 진행하지? 자동으로는 안되고 배포 시마다 알려줘야 할 거 같은데
	- 기능이 하나의 서비스에서만 동작하는게 아니라 여러 서비스가 같이 배포되어야 원하는 동작이 되는 경우일때.
		- 이 경우는 역할이 명확히 분리되어 있는게 이상적이긴 하나. 같이 필요한 경우는 의식적인 판단이 필요할 듯 하다.
- 알림이 가면 리뷰를 하고 -> 알림이 쌓이면 이전 알림을 못볼 수 있다 -> 알림 확인은 됐는데 모종의 이유로 배포는 연기해야 하면 이 상황을 다시 전달 -> 이벤트가 다시 전달되고 -> 다시 알림 온다
...
- docker repository
    ...
    ```
- artifect
	- java build 시 artifact 옵션 추가해서 빌드 산출물을 전달할 때 쓸 수 있다.
	- artifact에서 untracked 옵션은 git에 untracked 된 것들을 담을지 여부다.
		- 근데 이걸 false로 하니까 새로 만든 폴더를 artifact로 못만든다. 빌드파일에 쓰고 싶으면 true로 해야 할 듯
		- 파이프라인 탭에서 artifact를 직접 다운받을 수 있는데 이걸 다운 안받게 하는 방법은 딱히 없는 것 같다. 괜히 안만들고 싶은데.. -> expire 시간을 짧게 하면 됨
	...
	- paths 는 어떤 것을 artifact 할지 인데, ./ 하면 gitlab의 현재 위치의 것들을 모두 한다. 그래서 특정 위치를 잡아줄 수 있다
...
- 선언형과 groovy같은 스크립티드 문법 중 선택해서 쓸 수 있는데, 선언형으로 안되는 것들이 있어서 스크립티드를 써야하는 경우가 있다고 한다.
	- Configuration as code 플러그인을 설치하면 코드로 설정 관리 가능
	- https://tech.kakao.com/2021/07/16/devops-for-msa/

#### CI에서 레포트 만들 때 들어갈 내용
- 소스 코드 정보
...
#### 배포 관리 시 인프라 레포와 코드 레포 분리
프로젝트 레포에는 젠킨스파일처럼 파일 하나만 폴더에 추가해서 pr요청하면 그 파일이 서버에 훅을 보내서 특정 작업들을 실행하도록 한다

파일에는 세부 설정도 가능하지만 기능 on/off 정도만 해도 되도록 하면 좋지 않을까
argocd라고 적어놓으면 배포자동화가 되도록

코드 푸시하는 곳에서 이슈 리스트를 확인하고 코드를 내려받고, 푸시하면 리포트를 받아보게 되는 올인원 툴이 있으면 좋겠다. github action 추적도 되고

관리 쪽에서 개발소스를 컨텍해서 올리는게 더 안전성이 높지 않을까

argocd에서 쿠버네티스 템플릿을 가지고 있고, 코드 레포지토리에서는 이미지만 빌드하게 한다.
- 인프라 레포와 코드 레포의 분리.
...
퍼플은 3단계 레포 구성으로 가고 있다
- 전체 인프라 레포 (tf)
- Argocd 배포 관리 레포 (deploy-repo)
- 프로젝트 레포 (app)
...
#### 인프라용 cookiecutter (처음 프로젝트 만들 때 세팅 해주는 도구. 리액트에서 cra 처럼)
- ci/cd pipeline
- ansible script
- grafana prometheus
- fluentd
...
- 인프라를 따로 두면 각자 설정해야만 하는 것: 서버 주소나 git 주소
